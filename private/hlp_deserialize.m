...
for idx = 1:nobjects
    ...
    try
        ...
        v(idx) = eval([cls '.loadobj(conts)']);
    catch
        try
            % pass the struct directly to the constructor
            v(idx) = eval([cls '(conts)']);
        catch
            try
                ...
                v(idx) = conts;
                v(idx).hlp_deserialize_failed = ['could not construct class: ' cls];
            end
        end
    end
end
...
switch kind
    case 151 % simple function
        ...
        safe_name = strrep(name, '.', '__'); % dots are not valid in field names
        try
            % look up from table
            v = db_simple.(safe_name);
        catch
            % otherwise generate & fill table
            v = str2func(name);
            db_simple.(safe_name) = v;
        end
